...
const Form = () => {
  ...
  const handleSearch = (event) => {
    ...
    if (departureCity) {
      filteredFlights = filteredFlights.filter((flight) => flight.departureAirport.city.includes(departureCity));
    }

    if (destinationCity) {
      filteredFlights = filteredFlights.filter((flight) => flight.arrivalAirport.city.includes(destinationCity));
    }
  ...
  return (
    <div className="flex-grow flex items-center justify-center mt-0 mb-5 z-10 ">
      <div className="bg-white p-8 rounded shadow-md w-full ml-10 mr-10">
        ...
        <select className="rounded border p-2 flex-1 ml-0" value={flightClass} onChange={(e) => setFlightClass(e.target.value)}>
        ...
        {filteredFlights.length > 0 ? (
          ...
          isSearched && (
            <div>
              <p className="text-center font-bold text-gray-600">Wah penerbangan yang kamu cari gak ada, nih</p>
              <p className="text-center text-gray-600">Coba ganti tanggal atau cari lainnya di FlightMate</p>
            </div>
          )
        )}
      </div>
